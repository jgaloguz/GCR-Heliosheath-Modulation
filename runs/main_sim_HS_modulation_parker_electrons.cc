...
// Lower bound for momentum
   double momentum1 = Mom(3.0 * SPC_CONST_CGSM_MEGA_ELECTRON_VOLT / unit_energy_particle, specie);
   container.Insert(momentum1);

// Upper bound for momentum
   double momentum2 = Mom(100.0 * SPC_CONST_CGSM_MEGA_ELECTRON_VOLT / unit_energy_particle, specie);
...
// Import diffusion parameters
   ...
   std::ifstream diff_params_file("params_e.dat");
...
// Rigidity normalization factor
   double R0 = 0.33e9 / unit_rigidity_particle;
   container.Insert(R0);
...
// Pass ownership of "diffusion" to simulation
   simulation->AddDiffusion(DiffusionPotgieterEtAl2015(), container);
...
//! Spectral power law after bend
   double pow_law_Tb = -2.68;
   container.Insert(pow_law_Tb);

//! Smoothness of bend
   double bend_smooth = 1.0;
...
//----------------------------------------------------------------------------------------------------------------------------------------------------
   ...
   std::filesystem::create_directory("../results/HS_mod_spec_e");
   std::string simulation_files_prefix = "../results/HS_mod_spec_e/HS_mod_parker_" + std::to_string(perc_seg) + "_pct_";
